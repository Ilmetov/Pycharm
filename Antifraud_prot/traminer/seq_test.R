...
fraud <- read.csv(file = "traminer_fraud.csv", sep =";", header = TRUE, na.strings = "")
fraud_seq <-as.data.frame(fraud)
#fraud_seq <- fraud_seq[ -c(2,3,5,7,8) ]
fraud_seq$Index <- ave( 1:nrow(fraud_seq), fraud_seq$event_id, FUN=function(x) 1:length(x) )
max1 <- max(fraud_seq$Index, na.rm = TRUE)
fraud_sts <- TSE_to_STS(fraud_seq, id = 1, timestamp = 4, event = 3, stm = NULL, tmin = 1,
                        tmax = 4, firstState = "None")


my_seq_test <- read.csv(file = "seq_test.csv", sep =";", header = TRUE, na.strings = "")
my_seq_test2 <- seqdef(my_seq_test, var = 3:6, missing = "%") # missing = "%" чтобы удалять пропуски в середине как void
my_seq_test3 <- seqdef(my_seq_test2, left = "DEL", gaps = "DEL", right = "DEL")
